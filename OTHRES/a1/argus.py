...
tools = [
    ...
    {
        "number": "53",
        "name": "Run All Infrastructure Tools",
        "script": "",
        "section": "Run All Scripts",
    },
    {
        "number": "54",
        "name": "Run All Web Intelligence Tools",
        "script": "",
        "section": "Run All Scripts",
    },
    {
        "number": "55",
        "name": "Run All Security Tools",
        "script": "",
        "section": "Run All Scripts",
    },
    # Special Mode
...
number_of_modules = len(
    [
        tool
        for tool in tools
        if tool["script"] and tool["section"] not in ["Run All Scripts", "Special Mode"]
    ]
)
...
Version: [bold green]{VERSION}[/bold green]    Modules: [bold yellow]{number_of_modules}[/bold yellow]    Coded by: [bold magenta]{AUTHOR}[/bold magenta]
    ...
    console.print(
        Panel(combined_text, border_style=panel_color, padding=(1, 4)), justify="center"
    )
...
def display_table():
    ...
    table.add_column(
        "Network & Infrastructure", justify="left", style="cyan", no_wrap=True
    )
    table.add_column(
        "Web Application Analysis", justify="left", style="green", no_wrap=True
    )
    table.add_column(
        "Security & Threat Intelligence", justify="left", style="magenta", no_wrap=True
    )
    ...
    for tool in tools:
        if tool["section"] in sections:
            tools_by_section[tool["section"]].append(
                f"[bold]{tool['number']}[/bold]. {tool['name']}"
            )
    ...
    table.add_row(
        "[bold]53[/bold]. Run All Infrastructure Tools",
        "[bold]54[/bold]. Run All Web Intelligence Tools",
        "[bold]55[/bold]. Run All Security Tools",
    )

    table.add_row("", "", "")
    table.add_row(
        "", "[bold red]" + "-" * 15 + " 00. BEAST MODE " + "-" * 15 + "[/bold red]", ""
    )
...
def beast_mode():
    clear_screen()
    console.print(
        "[bold red][*] Running BEAST MODE - Executing All Modules Except Excluded Ones[/bold red]"
    )
    ...
    selected_modules = [
        ...
        for tool in tools
        if tool["script"]
        and tool["script"] not in excluded_scripts
        and tool["number"] != "00"
    ]
...
def execute_script(script_name, target):
    ...
    if os.path.isfile(script_path):
        try:
            with console.status(
                f"[bold green]Running {script_name}...[/bold green]", spinner="dots"
            ) as status:
        ...
        console.print(
            f"Script {script_name} not found in 'modules' directory.", style="bold red"
        )
...
def run_modules(selected_modules, api_status):
    ...
    generate_report(
        report_data, domain, [tools_mapping[mod]["name"] for mod in selected_modules]
    )
        ...
        while True:
            ...
            if choice == "00":
                ...
                selected_modules = [
                    tool["number"]
                    for tool in tools
                    if tool["section"] == "Network & Infrastructure"
                ]
                ...
                selected_modules = [
                    tool["number"]
                    for tool in tools
                    if tool["section"] == "Web Application Analysis"
                ]
                run_modules(selected_modules, check_api_modules())
                ...
                selected_modules = [
                    tool["number"]
                    for tool in tools
                    if tool["section"] == "Security & Threat Intelligence"
                ]
                run_modules(selected_modules, check_api_modules())
            else:
                selected_modules = [
